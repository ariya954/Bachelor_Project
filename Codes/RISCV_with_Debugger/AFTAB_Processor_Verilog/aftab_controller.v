...
module aftab_controller(
    ...
    // debugger's inputs
    input read_register_request_from_debugger,
    input write_register_request_from_debugger,
    input read_memory_request_from_debugger,
    input write_memory_request_from_debugger,
    ...
    output reg core_busy,
        output reg load_debugger_command,
        output reg load_debugger_command_data_argument,
	output reg load_debugger_command_address_argument,
        output reg select_given_register_number_from_debugger,
        output reg send_read_value_from_registerFile_to_debugger,
        output reg store_data_argument_from_debugger_in_registerFile,
	output reg select_given_memory_address_from_debugger,
	output reg send_read_value_from_memory_to_debugger,
	output reg store_data_argument_from_debugger_in_memory,
    ...
    always @(p_state, completeDARU, completeDAWU, completeAAU,
             ...
             exceptionRaise, interruptRaise, core_halt_request, debug_request, dividedByZeroOut,
    ...
    begin
        ...
        case(p_state)
            ...
            `updateUstatus:
                n_state <= `fetch;
            `waitForDebuggerRequest: begin
                if (~core_halt_request)
                    n_state <= `fetch;
    ...
    always @(p_state, exceptionRaise, interruptRaise, instrMisalignedOut,
             completeDARU, completeDAWU, completeAAU, opcode, func3, func12, func7,
    ...
    begin
        {selPC, selI4, selP2, selPCJ, selADR, selJL, selImm, selAdd, selInc4PC, selBSU,
		 ...
		 loadMieUieField, loadMieReg, load_debugger_command, load_debugger_command_data_argument,
	         load_debugger_command_address_argument, select_given_register_number_from_debugger, 
		 select_given_memory_address_from_debugger, send_read_value_from_registerFile_to_debugger, 
		 send_read_value_from_memory_to_debugger, store_data_argument_from_debugger_in_registerFile,
		 store_data_argument_from_debugger_in_memory} <= 0;
        // FPU
        ...
        case(p_state)
            `fetch: begin
                ...
                else if(~core_halt_request) begin
                    nBytes <= 2'b11;
                    selPCJ <= 1'b1;
                    dataInstrBar <= 1'b0;
                    selPC <= 1'b1;
